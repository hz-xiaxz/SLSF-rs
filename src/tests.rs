...
#[test]
fn layer_magnetization_and_interlayer_products() {
    ...
    assert!(cold.pair.iter().all(|row| row.len() == 3));
    assert!(cold.pair.iter().flatten().all(|v| (v - 1.0).abs() < 1e-12));

    // Rotate each layer rigidly: |m_z|^2 stays 1, products become cos of angle differences.
    ...
    for z in 0..5 {
        assert_abs_diff_eq!(rotated.pair[z][0], 1.0, epsilon = 1e-12);
        for r in 1..3 {
            let expected = (angles[z] - angles[(z + r) % 5]).cos();
            assert_abs_diff_eq!(rotated.pair[z][r], expected, epsilon = 1e-12);
        }
    }

    // Random angles: global M^2 is recovered from the stored products (r and L_z - r).
    initialize_angles(&mut lat, InitMode::Random, &mut rng).unwrap();
    let random = measure_layer_magnetization(&lat);
    let l_z = lat.l_z;
    let product = |z: usize, r: usize| {
        if r <= l_z / 2 {
            random.pair[z][r]
        } else {
            random.pair[(z + r) % l_z][l_z - r]
        }
    };
    let from_layers: f64 = (0..l_z)
        .flat_map(|z| (0..l_z).map(move |r| (z, r)))
        .map(|(z, r)| product(z, r))
        .sum::<f64>()
        / (l_z * l_z) as f64;
    let obs = measure_theta_observables(&lat, &params);
